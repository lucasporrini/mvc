      ...
      <ul class="space-y-1.5">
        <?php foreach($header_informations['menu'] as $item) { 
            if($item['is_parent'] == 1 && $item['has_child'] == 0) { ?>
                <li>
                    <a class="w-full flex items-center gap-x-3.5 py-2 px-2.5 text-sm text-slate-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-900 dark:text-slate-400 dark:hover:text-slate-300 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="<?= $item['url'] ?>">
                        <?= $item['name'] ?>
                    </a>
                </li>
        <? }
