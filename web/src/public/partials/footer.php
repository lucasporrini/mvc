<footer class="footer p-10 bg-base-300 text-base-content">
  ...
  <div>
    ...
    <a class="link link-hover" href="#">Cookies policy</a>
  </div>
</footer>
<footer class="footer px-10 py-4 border-t bg-base-200 text-base-content border-base-300">
  <aside class="items-center grid-flow-col">
    ...
    <?php
      foreach($header_informations['company'] as $el):
        if($el['parameter'] == "name") {
          $company['name'] = $el['value'];
        } else if ($el['parameter'] == "address") {
          $company['address'] = $el['value'];
        }
      endforeach;
    ?>
    <p class="md:place-self-center md:justify-self-end"><?= $company['name'] ?><br/><?= $company['address'] ?></p>
  </aside> 
