    ...
    <div id="navbar-collapse-with-animation" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow sm:block">
      <div class="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:ps-7">
        ...
        foreach ($menu as $item) {
          if ($item['active'] == 1 && $item['is_parent'] == 1) {
            // On verifie si l'item n'a pas de sous menu
            if ($item['has_child'] == 0) {
              echo '<a class="font-medium text-gray-500 hover:text-gray-400 sm:py-6 dark:text-gray-400 dark:hover:text-gray-500" href="' . $item['url'] . '">' . $item['name'] . '</a>';
            } else {
              echo '<div class="hs-dropdown [--strategy:static] sm:[--strategy:fixed] [--adaptive:none] sm:[--trigger:hover] sm:py-4">
              ...
              foreach($menu as $child) {
                if($child['is_parent'] == 0 && $child['parent_id'] == $item['id']) {
                  echo '<a class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-gray-100 focus:ring-2 focus:ring-blue-500 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-gray-300 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="' . $child['url'] . '">
                    ' . $child['name'] . '
                  </a>';
                }
              }
